...
bool MISC::getPoseFromInsWindow(const std::deque<std::pair<IMU, IntegrationState>> &window, const Pose &pose_ext,
    ...
    if (index > 0) {
        ...
        pose = stateToPoseWithExtrinsic(state, pose_ext);
        return true;
    } else {
        pose = stateToPoseWithExtrinsic(window.back().second, pose_ext);
        return false;
    }
}
...
Pose MISC::stateToPoseWithExtrinsic(const IntegrationState &state, const Pose &pose_ext) {
    Pose pose;

    pose.t = state.p + state.q.toRotationMatrix() * pose_ext.t;
    pose.R = state.q.toRotationMatrix() * pose_ext.R;
    return pose;
}
